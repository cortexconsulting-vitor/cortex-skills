...
Procure `marca.json` na pasta do projeto. **Se não existir, faça a entrevista antes
de qualquer outra coisa.** Uma pergunta por vez, esperando a resposta.
...
Grave `marca.json` seguindo `marca.exemplo.json`. Mostre o que gravou.

## Passo 2 — escrever o conteúdo

Crie a pasta `carrossel-<tema>-<AAAA-MM-DD>/`, copie `marca.json` pra dentro e
escreva `conteudo.json`.
